...
export default function LoginModal({ open, onClose,email, password,setEmail,setPassword,onSubmit,loading,loginMessage,message = null }) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
        <Box sx={style}>
          ...
          <Typography variant="h2" sx={{fontSize :'32px',fontWeight:'600',fontFamily:'Barlow'}}>Log in or Sign up</Typography>
          <Typography variant="body1" sx={{fontSize :'18px',fontFamily:'Barlow',marginBottom:'30px'}}>{message || 'Login or sign up to complete your booking'}</Typography>
          <Box display='flex' flexDirection='column' gap='30px' sx={{width:'100%'}}>
              ...
              <Box display='flex' justifyContent='center' gap='10px' alignItems='center'>
                  <form style={{width:'100%'}} onSubmit={handleFormSubmit}>
                      ...
                      <Button
                          ...
                          disabled={loading}
                          >
                            {loading ? (<CircularProgress size="30px" />) : ('Continue')}
                      </Button>
                  </form>
              </Box>
              ...
              >
                ...
                <Link to={`${ROUTES.customerLogin}?redirectTo=${currentPath}`}>
                  <span
