...
const textFieldSx = {
  '& .MuiOutlinedInput-root': {
    borderRadius: '10px',
    background: '#FFFFFF',
    '& fieldset': { borderColor: '#E7DACB' },
    '&:hover fieldset': { borderColor: '#4E2E45' },
    '&.Mui-focused fieldset': { borderColor: '#4E2E45' },
  },
  '& .MuiInputLabel-root.Mui-focused': { color: '#4E2E45' },
};
...
const primaryButtonSx = {
  ...
  fontSize: '15px',
  boxShadow: 'none',
  '&:hover': { background: '#3c2337', boxShadow: 'none' },
};
...
function ProfessionalLoginPage() {
    ...
    const handleEmailCheck = async (e) => {
        e.preventDefault();
        setLoading(true);
        const payload = { email: email, roles: ['owner', 'worker'] }
        try {
    ...
    return (
        <Box className="bt-auth">
            ...
            <div className="bt-auth__content bt-auth__content--center">
                {showAlert && (
                    <Alert
                ...
                {errors.length > 0 && (
                    <Alert className="bt-auth-alert" severity="error" sx={{ mb: 2 }}>
                ...
                {showForm === 'basic' && (
                    <>
                        <span className="bt-auth__eyebrow">Professionals</span>
                        <h1 className="bt-auth__title">Welcome to <span>BeautyTrafic</span></h1>
                        <p className="bt-auth__subtitle">Create an account or log in to manage your business.</p>

                        <div className="bt-auth-form">
                            <form className="bt-auth-fields" onSubmit={handleEmailCheck}>
                                <TextField
                                    label="Email address"
                                    variant="outlined"
                                    fullWidth
                                    ...
                                    sx={textFieldSx}
                                />
                                <Button type="submit" variant="contained" fullWidth sx={primaryButtonSx} disabled={loading}>
                                    Continue
                                </Button>
                            </form>

                            <div className="bt-auth-divider"><span>or</span></div>

                            <Link
                                className="bt-auth-social"
                                to={`${process.env.REACT_APP_LARAVEL_BASE_URL}/auth/google/redirect?login_type=owner`}
                            >
                                <span className="bt-auth-social__icon">
                            ...
                            <p className="bt-auth-switch">
                                Looking to book an appointment?
                                <Link to={ROUTES.customerLogin} className="bt-auth-switch__link">Go to BeautyTrafic for customers</Link>
                            </p>
                        </div>
                    </>
                )}
                ...
                {showForm === 'password' && (
                    <>
                        <span className="bt-auth__eyebrow">Professionals</span>
                        <h1 className="bt-auth__title">Welcome <span>back</span></h1>
                        <p className="bt-auth__subtitle">Enter your password to log in as {email}</p>

                        <div className="bt-auth-form">
                            <form className="bt-auth-fields" onSubmit={handleLogin}>
                                <TextField
                                    label="Password"
                                    variant="outlined"
                                    fullWidth
                            ...
                            <p className="bt-auth-note">
                                Forgot password?{' '}
                                <span className="bt-auth-note__action" onClick={forgotPasswordHandle}>Click here to reset.</span>
                            </p>
                        </div>
                    </>
                )}
                ...
                {showForm === 'completeForm' && (
                    <>
                        <span className="bt-auth__eyebrow">Professionals</span>
                        <h1 className="bt-auth__title">Create <span>account</span></h1>
                        <p className="bt-auth__subtitle">You're almost there! Complete these details for {email}</p>

                        <div className="bt-auth-form">
                            <form className="bt-auth-fields" onSubmit={handleSignup}>
                                <TextField label="Name" variant="outlined" fullWidth margin="normal" type="text" value={username} onChange={(e) => setUsername(e.target.value)} required disabled={loading} sx={textFieldSx} />
                                <TextField label="Phone number" variant="outlined" fullWidth margin="normal" type="number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} required disabled={loading} sx={textFieldSx} />
                                <TextField label="Password" variant="outlined" fullWidth margin="normal" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required disabled={loading} sx={textFieldSx} />
                                <TextField label="Confirm password" variant="outlined" fullWidth margin="normal" type="password" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} required disabled={loading} sx={textFieldSx} />

                                <Button variant="outlined" component="label" fullWidth className="bt-auth-upload" sx={{ mt: 2 }}>
                                    Upload profile image
                                    <input type="file" accept="image/*" hidden onChange={handleFileChange} />
                                </Button>
                                {profileImage && (
                                    <Typography variant="body2" sx={{ mt: 1, color: 'var(--bt-ink-soft)' }}>
                                ...
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            ...
                                            disabled={loading}
                                            sx={{ color: '#4E2E45', '&.Mui-checked': { color: '#4E2E45' } }}
                                        />
                                    }
                                ...
                                <Button type="submit" variant="contained" fullWidth sx={primaryButtonSx} disabled={loading || !acceptedTerms}>
                                    Sign up
                                </Button>
                            </form>
                        </div>
                ...
                {showForm === 'verifyEmail' && (
                    <>
                        <span className="bt-auth__eyebrow">Professionals</span>
                        <h1 className="bt-auth__title">Enter <span>code</span></h1>
                        <p className="bt-auth__subtitle">Enter the verification code we've sent to {email}</p>

                        <div className="bt-auth-form">
                            <form className="bt-auth-fields" onSubmit={handleVerification}>
                                <TextField
