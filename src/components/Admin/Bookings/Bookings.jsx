...
function AdminBookingsPage() {
  ...
  const handleStatusChangeStatus = async (id, newStatus) => {
    setLoading(true);
    try {
      const { data } = await axiosClient.put(`/updateBookingStatus/${id}`, {
        status: newStatus,
      });
  ...
  const handleBookingFilter = (filter) => {
    ...
    const filtered =
      key === "all" || key === "total bookings"
        ? bookings
        : bookings.filter((b) => b.status === key);
    if (view === "list") {
  ...
  useEffect(() => {
    ...
    const pendingPast = bookings.filter((b) => {
      const bDate = new Date(
        ...
        parseInt(b.booking_date.split("-")[1]) - 1,
      ...
      >
        ...
        <span
          style={{
            ...
            height: "8px",
            borderRadius: "50%",
            background: getStatusDot(booking?.status),
            verticalAlign: "middle",
          }}
        />
      </div>
        ...
        >
          {statCards.map((s) => (
            ...
            >
              ...
              <div
                style={{ fontSize: "22px", fontWeight: 600, color: s.color }}
              >
                {s.val}
              </div>
            </div>
        ...
        {view === "list" && (
          <div style={S.card}>
            <div style={S.bulkBar}>
              ...
              <button
                style={
                  selectedRows.length === 0 || !bulkStatus
                    ? S.applyBtnDisabled
                    : S.applyBtn
                }
                ...
                {filteredBookings.length > 0 ? (
                  filteredBookings.map((b, i) => (
                    <tr
                      key={b.id}
                      style={{
                        background: selectedRows.includes(b.id)
                          ? "#f0f4ff"
                          : i % 2 === 0
                            ? "#fff"
                            : "#fafaf8",
                    ...
                    >
                      ...
                      <td style={S.td}>
                        ...
                        {" – "}
                        {new Date(
                          `1970-01-01T${b.booking_time_end}`,
                        ).toLocaleTimeString([], {
                          hour: "numeric",
                          minute: "2-digit",
                        })}
